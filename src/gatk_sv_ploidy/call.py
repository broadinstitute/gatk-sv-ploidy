...
from gatk_sv_ploidy._util import (
    ...
    expected_allosome_copy_number_pairs,
...
_DEFAULT_ALLOSOME_PROB_THRESHOLD = 0.5
...
def _preferred_depth_map(sdf: pd.DataFrame) -> dict[str, float]:
    """Return per-chromosome depth summaries, preferring plot-normalized depth."""
    depth_values: dict[str, float] = {}
    plot_depths = None
    if "plot_median_depth" in sdf.columns:
        plot_depths = sdf["plot_median_depth"]
    median_depths = sdf["median_depth"] if "median_depth" in sdf.columns else None

    for row in sdf.itertuples(index=False):
        chrom = str(row.chromosome)
        depth_value = float("nan")
        if plot_depths is not None:
            plot_value = float(plot_depths.loc[sdf.index[sdf["chromosome"] == chrom][0]])
            if np.isfinite(plot_value):
                depth_value = plot_value
        if (not np.isfinite(depth_value)) and median_depths is not None:
            median_value = float(median_depths.loc[sdf.index[sdf["chromosome"] == chrom][0]])
            if np.isfinite(median_value):
                depth_value = median_value
        depth_values[chrom] = depth_value

    return depth_values


def _infer_expected_allosome_pair_from_depth(
    x_depth: float,
    y_depth: float,
    autosomal_baseline_cn: int,
) -> tuple[int, int] | None:
    """Infer the nearest expected allosome CN pair from chrX/chrY depth."""
    female_like, male_like = expected_allosome_copy_number_pairs(autosomal_baseline_cn)
    candidate_pairs = (female_like, male_like)
    ...
    for candidate_x, candidate_y in candidate_pairs:
        ...
        if np.isfinite(x_depth):
            distance += (x_depth - float(candidate_x)) ** 2
            used_dimension = True
        if np.isfinite(y_depth):
            distance += (y_depth - float(candidate_y)) ** 2
            used_dimension = True
        if not used_dimension:
            continue
        if distance < best_distance:
            best_distance = distance
            best_pair = (int(candidate_x), int(candidate_y))

    return best_pair


def _resolve_allosome_copy_numbers(
    sdf: pd.DataFrame,
    raw_cn_map: dict[str, int],
    depth_map: dict[str, float],
    autosomal_baseline_cn: int,
    prob_threshold: float = _DEFAULT_ALLOSOME_PROB_THRESHOLD,
) -> tuple[int, int]:
    """Return confidence-aware chrX/chrY copy numbers for final classification."""
    x_cn = int(raw_cn_map.get("chrX", autosomal_baseline_cn))
    y_cn = int(raw_cn_map.get("chrY", 0))
    if is_expected_allosome_copy_number_pair(x_cn, y_cn, autosomal_baseline_cn):
        return x_cn, y_cn
    ...
    y_ok = ("chrY" not in prob_map) or (float(prob_map["chrY"]) > prob_threshold)
    if x_ok and y_ok:
        return x_cn, y_cn

    fallback_pair = _infer_expected_allosome_pair_from_depth(
        depth_map.get("chrX", float("nan")),
        depth_map.get("chrY", float("nan")),
...
def assign_sex_and_aneuploidy_types(
    ...
    allosome_prob_threshold: float = _DEFAULT_ALLOSOME_PROB_THRESHOLD,
) -> pd.DataFrame:
    ...
    for sample_id, sdf in df.groupby("sample"):
        ...
        depth_map = _preferred_depth_map(sdf)
        autosomal_baseline_cn = 2
        if "autosomal_baseline_cn" in sdf.columns:
            baseline_values = sdf["autosomal_baseline_cn"].dropna()
            if not baseline_values.empty:
        ...
        cn_scale_factor = 1.0
        x_cn, y_cn = _resolve_allosome_copy_numbers(
            sdf,
            raw_cn_map,
            depth_map,
            autosomal_baseline_cn,
            prob_threshold=allosome_prob_threshold,
        )
        cn_map["chrX"] = x_cn
        cn_map["chrY"] = y_cn
        aneu_map = {
        ...
        x_depth = depth_map.get("chrX", float(x_cn))
        y_depth = depth_map.get("chrY", float(y_cn))
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "If set, rebuild chromosome calls after excluding bins with the "
            "selected quality field below this threshold. By default this uses "
            "BINQ20; use --use-callq20 to filter on CALLQ20 instead."
        ),
    )
    p.add_argument(
