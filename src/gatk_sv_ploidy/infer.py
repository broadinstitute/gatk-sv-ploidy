                ...
                for sex_state, sex_label in ((0, "XX"), (1, "XY")):
                    ...
                    if bin_bias_matrix is not None:
                        ...
                        if "bin_var" in map_estimates:
                            ...
                            if corr is not None:
                                messages.append(
                                    (
                                        "Correlation(bin_var, "
                                        f"{chrom_label} mean absolute "
                                        "observed-minus-expected plot depth "
                                        f"among {sex_label}-assigned samples "
                                        f"by bin)={corr:.3f}"
                                    )
                                )
                        corr = _safe_correlation(
                        ...
                        if corr is not None:
                            messages.append(
                                (
                                    "Correlation(bin_bias, "
                                    f"{chrom_label} mean absolute "
                                    "observed-minus-expected plot depth "
                                    f"among {sex_label}-assigned samples by "
                                    f"bin)={corr:.3f}"
                                )
                            )
