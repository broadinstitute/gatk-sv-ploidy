...
def test_assign_falls_back_to_depth_for_low_confidence_allosome_pair() -> None:
    df = pd.DataFrame(
        [
            {
                "sample": "S1",
    ...
    assert pred_df.loc["S1", "sex"] == "FEMALE"
    assert pred_df.loc["S1", "predicted_aneuploidy_type"] == "NORMAL"
    assert int(pred_df.loc["S1", "chrX_CN"]) == 2
    assert int(pred_df.loc["S1", "chrY_CN"]) == 0
    assert int(pred_df.loc["S1", "raw_chrX_CN"]) == 1
